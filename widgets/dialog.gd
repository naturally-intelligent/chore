...
export(BubbleStyle) var bubble_style setget set_bubble_style
export(bool) var font_color_override = false
...
func _process(delta):
	...
	match dialog_action:
		'leave':
			...
			dialog_action = 'leave-on'
			var tween = Tween.new()
			var new_pos = get_position()
			new_pos.y = new_pos.y - get_size().y
			tween.interpolate_property(self,
				"rect_position", get_position(), new_pos, 0.5,
...
func connect_scene(scene):
	if scene.has_node('DialogBackButton'):
		...
		dbb.connect('pressed', self, 'dialog_press')
...
func build_dialog(text, type='dialog'):
	...
	if text_lines == 1 and type != 'choice': # vcenter text a bit for 1 line bubbles
		container.set("custom_constants/margin_top", 4)
	...
	if type == 'action':
		...
		bump_height += 34
		if text_lines == 1:
			container.set("custom_constants/margin_top", 8)
			bubble_height += 8
	...
	if click_removes:
		...
		button.visible = true
		button.connect('pressed', self, 'bubble_press', [dialog_bubble])
